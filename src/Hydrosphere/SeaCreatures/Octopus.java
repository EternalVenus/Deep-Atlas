...
public class Octopus extends SeaCreature {  // extend means it inherits from SeaCreature class
    ...
    public Octopus(String name, boolean camoflauge) {
        this.name = name;
        this.length = (int) (2 + Math.floor((Math.random() * 20)));
    ...
    public void motherOfTheYear() {
        System.out.println("having laid her eggs, the mother does not eat or leave her eggs. " +
                "She cares for the eggs and cleans them but dies shortly after they hatch.");
    }

    public void camoflauge() {
        if (camoflauge){
            System.out.println("The octopus blends into its surroundings using chromatophores in their skins. " +
